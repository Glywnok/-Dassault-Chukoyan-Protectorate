	...
	public void generateForSector(ThemeGenContext context, float allowedUnusedFraction) {
		...
		for (int i = 0; i < num && i < constellations.size(); i++) {
			...
			for (int j = 0; j < numMain; j++) {
				...
				data.system.addTag(dcp_Tags.THEME_BREAKER);
				if (type != BladeBreakerSystemType.DESTROYED) {
					data.system.addTag(Tags.THEME_UNSAFE);
				}
				data.system.addTag(dcp_Tags.THEME_BREAKER_MAIN);
				...
				if (type == BladeBreakerSystemType.DESTROYED) {
					...
					if (addStation) {
						List<CampaignFleetAPI> stations = addBattlestations(data, 1f, 1, 1, createStringPicker(dcp_Tags.BATTLESTATION_BREAKER_LIGHT, 10f));
					...
					List<CampaignFleetAPI> stations = addBattlestations(data, 1f, 1, 1, createStringPicker(dcp_Tags.BATTLESTATION_BREAKER_HEAVY, 10f));
					for (CampaignFleetAPI station : stations) {
			...
			for (StarSystemData data : systems) {
				...
				data.system.addTag(dcp_Tags.THEME_BREAKER_SECONDARY);
				data.system.addTag(type.getTag());
				breakerSystems.add(data);
				...
				if (random.nextFloat() < 0.5f) {
					...
					data.system.addTag(dcp_Tags.THEME_BREAKER_NO_FLEETS);
				}
			}
//			if (count == 18) {
...
//			}
			...
			for (AddedEntity added : data.generated) {
				...
				float min = 1f;
				float max = 1f;
				if (dcp_Entities.STATION_MINING_BREAKER.equals(added.entityType)) {
					prob = 0.25f;
					min = 8;
					max = 15;
				} else if (dcp_Entities.ORBITAL_HABITAT_BREAKER.equals(added.entityType)) {
					prob = 0.25f;
					min = 8;
					max = 15;
				} else if (dcp_Entities.STATION_RESEARCH_BREAKER.equals(added.entityType)) {
					prob = 0.25f;
					min = 10;
					max = 20;
				}
	...
	public void populateNonMain(StarSystemData data) {
		...
		if (special) {
			addResearchStations(data, 0.75f, 1, 1, createStringPicker(dcp_Entities.STATION_RESEARCH_BREAKER, 10f));
		}
		
		if (random.nextFloat() < 0.5f) return;
		
		if (!data.resourceRich.isEmpty()) {
			addMiningStations(data, 0.5f, 1, 1, createStringPicker(dcp_Entities.STATION_MINING_BREAKER, 10f));
		}
		
		if (!special && !data.habitable.isEmpty()) {
			// ruins on planet, or orbital station
			addHabCenters(data, 0.25f, 1, 1, createStringPicker(dcp_Entities.ORBITAL_HABITAT_BREAKER, 10f));
		}
		
		addShipGraveyard(data, 0.05f, 1, 1,
				createStringPicker(Factions.INDEPENDENT, 10f, Factions.SCAVENGERS, 8f, dcp_Factions.DASSAULT, 6f, Factions.PIRATES, 3f));
		...
		addCaches(data, 0.25f, 0, 2, createStringPicker(
                                dcp_Entities.WEAPONS_CACHE_BREAKER, 2f,
				dcp_Entities.WEAPONS_CACHE_SMALL_BREAKER, 5f,
				"supply_cache", 4f,
	...
	public void populateMain(StarSystemData data, BladeBreakerSystemType type) {
		...
		addHabCenters(data, 1, maxHabCenters, maxHabCenters, createStringPicker(
                        dcp_Entities.ORBITAL_HABITAT_BREAKER, 10f));
		...
		addMiningStations(data, probMining, 1, 1, createStringPicker(dcp_Entities.STATION_MINING_BREAKER, 10f));
		
		addResearchStations(data, probResearch, 1, 1, createStringPicker(dcp_Entities.STATION_RESEARCH_BREAKER, 10f));
...
//		}
		...
		addCaches(data, 0.75f, 0, 3, createStringPicker(
                                dcp_Entities.WEAPONS_CACHE_BREAKER, 5f,
				dcp_Entities.WEAPONS_CACHE_SMALL_BREAKER, 5f,
				"supply_cache", 10f,
				"supply_cache_small", 10f,
				"equipment_cache", 10f,
...
//				});
	...
	public static CustomCampaignEntityAPI addBeacon(StarSystemAPI system, BladeBreakerSystemType type) {
		...
		CustomCampaignEntityAPI beacon = Global.getSector().getHyperspace().addCustomEntity(null, null, "dcp_bladebreaker_beacon", Factions.NEUTRAL);
		//beacon.getMemoryWithoutUpdate().set("$breaker", true);
                //beacon.addTag(dcp_Tags.HARDENED_WARNING_BEACON); not sure this needs to be added AGAIN
