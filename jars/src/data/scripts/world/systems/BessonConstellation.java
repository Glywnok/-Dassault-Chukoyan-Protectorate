...
import data.campaign.ids.dcp_Commodities;
import data.campaign.ids.dcp_Conditions;
import data.campaign.ids.dcp_Entities;
import data.campaign.ids.dcp_Factions;
import data.campaign.ids.dcp_Tags;
import data.campaign.ids.dcp_Terrain;
...
import static data.scripts.world.SigmaUtils.DCP_GUARDIAN_ACOLYTE;
import static data.scripts.world.SigmaUtils.DCP_GUARDIAN_DMG;
import static data.scripts.world.SigmaUtils.DCP_GUARDIAN_STD;
...
public class BessonConstellation {
    ...
    public static float radius_derelictvar = 700f;
        ...
        StarAge dcp_constellation_Age = StarAge.ANY;

        if (selector < 0.33f) {
            dcp_constellation_Age = StarAge.YOUNG;
        }
        if (selector >= 0.33f && selector < 0.66f) {
            dcp_constellation_Age = StarAge.AVERAGE;
        }
        if (selector >= 0.66f) {
            dcp_constellation_Age = StarAge.OLD;
        }

        // create the constellation nebula       
        Constellation dcp_constellation_Besson = new Constellation(
                Constellation.ConstellationType.NORMAL, dcp_constellation_Age
        );
        ...
        dcp_constellation_Besson.setNamePick(constname);      // sets the new star name
        ...
        dcp_constellation_Besson.getSystems().add(sector.getStarSystem("Alpha " + StarName));
        dcp_constellation_Besson.getSystems().add(sector.getStarSystem("Beta " + StarName));
        dcp_constellation_Besson.getSystems().add(sector.getStarSystem("Gamma " + StarName));
        dcp_constellation_Besson.getSystems().add(sector.getStarSystem("Delta " + StarName));
        dcp_constellation_Besson.getSystems().add(sector.getStarSystem("Epsilon " + StarName));

        sector.getStarSystem("Alpha " + StarName).setConstellation(dcp_constellation_Besson);
        sector.getStarSystem("Beta " + StarName).setConstellation(dcp_constellation_Besson);
        sector.getStarSystem("Gamma " + StarName).setConstellation(dcp_constellation_Besson);
        sector.getStarSystem("Delta " + StarName).setConstellation(dcp_constellation_Besson);
        sector.getStarSystem("Epsilon " + StarName).setConstellation(dcp_constellation_Besson);
        ...
        system_besson.setBackgroundTextureFilename("graphics/backgrounds/background4.jpg");
			...
			system_besson, // location to add to
                        "terrain", "dcp_nebula_sigma", // "nebula_blue", // texture to use, uses xxx_map for map
		...
		system_besson.setLightColor(new Color(210, 230, 255)); // light color in entire system_besson, affects all entities
                ...
                system_besson.addTag(dcp_Tags.THEME_BREAKER);
                system_besson.addTag(dcp_Tags.THEME_BREAKER_MAIN);
                system_besson.addTag(dcp_Tags.THEME_BREAKER_RESURGENT);
                system_besson.addTag(dcp_Tags.THEME_BREAKER_HOMEWORLD);
		...
		besson_inner_loc.setCircularOrbitPointingDown(besson_star, 360*(float)Math.random(), 4800, 300f);
                ...
                PlanetAPI lenze = system_besson.addPlanet("dcp_planet_lenze", besson_star, "Lenze", "dcp_sigmaworld", 145, 150, 5600, -180);
		...
		lenze.getSpec().setShieldTexture(Global.getSettings().getSpriteName("industry", "dcp_shield_texture"));
		lenze.getSpec().setShieldThickness(0.06f);
		lenze.getSpec().setShieldColor(new Color(255,255,255,100));
                lenze.applySpecChanges();
                
                //SectorEntityToken lenzeStation = system_besson.addCustomEntity("dcp_lenze_port", "Lenze Atoll", "station_breaker", "blade_breakers");
                //lenzeStation.setCircularOrbitPointingDown(system_besson.getEntityById("dcp_planet_lenze"), 180, 350, 35);
                //lenzeStation.setInteractionImage("illustrations", "orbital");
                
                // Fake market for conditions.
                    Misc.initConditionMarket(lenze);
                    lenze.getMarket().addCondition(dcp_Conditions.SIGMARAD);
                ...
                // Lenze's various crazyass FX and radiation belts.
                    ...
                    system_besson.addCorona(lenze, dcp_Terrain.CORONA_SIGMA,
		...
		CustomCampaignEntityAPI beacon = system_besson.addCustomEntity(null, null, "dcp_bladebreaker_beacon", Factions.NEUTRAL);
		...
		beacon.getMemoryWithoutUpdate().set("$dcp_lenzewarn", true);
		...
		//And then use $dcp_lenzewarn as a condition for custom interaction text in rules.csv.
            
                // Spawn defense fleets around Lenze. Should be naaasty.
                FleetMemberAPI member1 = Global.getFactory().createFleetMember(FleetMemberType.SHIP, DCP_GUARDIAN_STD);
                ...
                FleetMemberAPI member2 = Global.getFactory().createFleetMember(FleetMemberType.SHIP, DCP_GUARDIAN_STD);
                ...
                FleetMemberAPI member3 = Global.getFactory().createFleetMember(FleetMemberType.SHIP, DCP_GUARDIAN_STD);
		...
		besson_l1_loc.setCircularOrbitPointingDown(besson_star, 300, 9600, 400f);
                ...
                marat.setCustomDescriptionId("planet_marat");
                    ...
                    // Moon #1
                    PlanetAPI marat1 = system_besson.addPlanet("dcp_planet_marat1", marat, "Marat I", "cryovolcanic", 75, 45, 540, 180);
                    ...
                    // Moon #2
                    PlanetAPI marat2 = system_besson.addPlanet("dcp_planet_marat2", marat, "Marat II", "frozen", 135, 90, 1100, 200);
                    ...
                    // Moon #3
                    PlanetAPI marat3 = system_besson.addPlanet("dcp_planet_marat3", marat, "Marat III", "barren", 215, 60, 1560, 220);
		...
		system_besson.setSecondary(besson_star_b);
                //besson_star_b.setCustomDescriptionId("star_lucille");
                    ...
                    // Lucille needs some moons.
                        // First moon.
                        PlanetAPI lucille1 = system_besson.addPlanet("dcp_planet_lucille1", besson_star_b, "Alpha " + StarName + " B I", "barren-bombarded", 360*(float)Math.random(), 40, 1280, 140);
                        ...
                        PlanetAPI lucille2 = system_besson.addPlanet("dcp_planet_lucille2", besson_star_b, "Source", "terran-eccentric", 360*(float)Math.random(), 60, 1600, 180);
...
//                        SectorEntityToken stationDerelict2 = DerelictThemeGenerator.addSalvageEntity(system_besson, dcp_Entities.STATION_RESEARCH_BREAKER, Factions.DERELICT);
...
//                            Misc.setDefenderOverride(stationDerelict2, new DefenderDataOverride("blade_breakers", 1f, 15, 36));
                    ...
                    SectorEntityToken stationDerelict1 = DerelictThemeGenerator.addSalvageEntity(system_besson, dcp_Entities.STATION_MINING_BREAKER, dcp_Factions.BREAKERS);
                    ...
                    extraStation1Salvage.addCommodity(dcp_Commodities.SIGMA_MATTER_HIGH, 1);
                    extraStation1Salvage.addCommodity(dcp_Commodities.SIGMA_MATTER_LOW, 3);
                    extraStation1Salvage.addCommodity(dcp_Commodities.SIGMA_MATTER_UNSTABLE, 6);
                    BaseSalvageSpecial.addExtraSalvage(extraStation1Salvage, stationDerelict1.getMemoryWithoutUpdate(), -1);                    
                    
                    // Something fun at L5.
                    SectorEntityToken l5scrap = DerelictThemeGenerator.addSalvageEntity(system_besson, dcp_Entities.WEAPONS_CACHE_BREAKER, dcp_Factions.BREAKERS);
                    ...
                    //Derelict Lilin.
                    addDerelict(system_besson, l5scrap, "dcp_bbminerunner_std", ShipRecoverySpecial.ShipCondition.AVERAGE, 180f, true, "the_deserter");

        
                // Abandoned research station at L2.
                SectorEntityToken stationDerelict3 = DerelictThemeGenerator.addSalvageEntity(system_besson, dcp_Entities.STATION_RESEARCH_BREAKER, dcp_Factions.BREAKERS);
		...
		extraStation2Salvage.addCommodity(Commodities.ALPHA_CORE, 1);
                extraStation2Salvage.addCommodity(dcp_Commodities.SIGMA_MATTER_LOW, 2);
		BaseSalvageSpecial.addExtraSalvage(extraStation2Salvage, stationDerelict3.getMemoryWithoutUpdate(), -1);
                
                // Planetoid for spacing?
                PlanetAPI besson4 = system_besson.addPlanet("dcp_planet_besson4", besson_star, "Alpha " + StarName + " IV", "barren", 360*(float)Math.random(), 60, 17600, 480);
                ...
                bessonHabitat.setSensorProfile(0.3f);
                    ...
                    bessonHabitat.getMarket().getSubmarket(Submarkets.SUBMARKET_STORAGE).getCargo().addSpecial(new SpecialItemData("dcp_sigmatech_package", null), 1f);
                    bessonHabitat.getMarket().getSubmarket(Submarkets.SUBMARKET_STORAGE).getCargo().addSpecial(new SpecialItemData("dcp_deserter_package", null), 1f);
                    bessonHabitat.getMarket().getSubmarket(Submarkets.SUBMARKET_STORAGE).getCargo().addSpecial(new SpecialItemData("dcp_bladewood_package", null), 1f);
                    //bessonHabitat.getMarket().getSubmarket(Submarkets.SUBMARKET_STORAGE).getCargo().addMothballedShip(FleetMemberType.SHIP, "dcp_stormkestrel_proto_test", "Unit 01");
                    //bessonHabitat.getMarket().getSubmarket(Submarkets.SUBMARKET_STORAGE).getCargo().addMothballedShip(FleetMemberType.SHIP, "dcp_curse_proto_test", "Unit 02");
        ...
        system_two.setLightColor(new Color(255, 225, 205)); // light color in entire system, affects all entities
            ...
            system_two.addTag(dcp_Tags.THEME_BREAKER);
            system_two.addTag(dcp_Tags.THEME_BREAKER_MAIN);
            system_two.addTag(dcp_Tags.THEME_BREAKER_SUPPRESSED);
        ...
        StarSystemGenerator.addSystemwideNebula(system_two, dcp_constellation_Age);
        
            // Add single Acolyte
            FleetMemberAPI member4 = Global.getFactory().createFleetMember(FleetMemberType.SHIP, DCP_GUARDIAN_ACOLYTE);
        ...
        addDerelict(system_two, two_star, "dcp_curse_proto_test", ShipRecoverySpecial.ShipCondition.GOOD, radius_curse, true, "the_deserter");
                
        //Add a few more derelicts to the Cursed Dust Band, including a few prototype Imps (one recoverable).
                addDerelict(system_two, two_star, "dcp_imp_proto_test", ShipRecoverySpecial.ShipCondition.BATTERED, radius_curse - 100, true, "the_deserter");                
                ...
                addDerelict(system_two, two_star, "dcp_puddlejumper_mk1_std", ShipRecoverySpecial.ShipCondition.WRECKED, radius_curse - 75f, false, "independent");                
        ...
        PlanetAPI laureline = system_two.addPlanet("dcp_planet_laureline", two_star, "Laureline", "jungle", 300, 120, 6400, 210f);
		...
		extraStation3Salvage.addCommodity(dcp_Commodities.SIGMA_MATTER_UNSTABLE, 3);
        ...
        system_three.setLightColor(new Color(255, 225, 205)); // light color in entire system, affects all entities
            ...
            //This is where I add tags.
            system_three.addTag(dcp_Tags.THEME_BREAKER);
            system_three.addTag(dcp_Tags.THEME_BREAKER_MAIN);
            system_three.addTag(dcp_Tags.THEME_BREAKER_SUPPRESSED);
        ...
        StarSystemGenerator.addSystemwideNebula(system_three, dcp_constellation_Age);
        
            // Add single Acolyte
            FleetMemberAPI member5 = Global.getFactory().createFleetMember(FleetMemberType.SHIP, DCP_GUARDIAN_ACOLYTE);
        ...
        system_four.setLightColor(new Color(255, 225, 205)); // light color in entire system, affects all entities
            ...
            //This is where I add tags.
            system_four.addTag(dcp_Tags.THEME_BREAKER);
            system_four.addTag(dcp_Tags.THEME_BREAKER_MAIN);
            //system_four.addTag(dcp_Tags.THEME_BREAKER_DESTROYED);
            system_four.addTag(dcp_Tags.THEME_BREAKER_SUPPRESSED);
        ...
        StarSystemGenerator.addSystemwideNebula(system_four, dcp_constellation_Age);
        
            //Accretion disk code. Implement your own blank texture in place of 'dcp_blankring'.
            system_four.addRingBand(four_star, "misc", "dcp_blankring", 256f, 0, Color.BLACK, 2160f, 1900, 69f, Terrain.RING, "Delta " + StarName + " Disk");
            ...
            SectorEntityToken stationMining1 = DerelictThemeGenerator.addSalvageEntity(system_four, dcp_Entities.STATION_MINING_BREAKER, dcp_Factions.BREAKERS);
            ...
            extraMining1Salvage.addCommodity(dcp_Commodities.SIGMA_MATTER_LOW, 2);
            extraMining1Salvage.addCommodity(dcp_Commodities.SIGMA_MATTER_UNSTABLE, 5);
            BaseSalvageSpecial.addExtraSalvage(extraMining1Salvage, stationMining1.getMemoryWithoutUpdate(), -1); 
                    
            // Add single Acolyte
            FleetMemberAPI member6 = Global.getFactory().createFleetMember(FleetMemberType.SHIP, DCP_GUARDIAN_ACOLYTE);
        ...
        system_five.setLightColor(new Color(255, 210, 200)); // light color in entire system, affects all entities
            ...
            //This is where I add tags.
            system_five.addTag(dcp_Tags.THEME_BREAKER);
            system_five.addTag(dcp_Tags.THEME_BREAKER_MAIN);
            system_five.addTag(dcp_Tags.THEME_BREAKER_SUPPRESSED);
        ...
        StarSystemGenerator.addSystemwideNebula(system_five, dcp_constellation_Age);
        
            // Add single damaged Preceptor.
            FleetMemberAPI member7 = Global.getFactory().createFleetMember(FleetMemberType.SHIP, DCP_GUARDIAN_DMG);
            ...
            PlanetAPI yod2 = system_five.addPlanet("dcp_planet_yod2", five_star, "Dzerzhinsky", "gas_giant", 300, 450, 2800, 300f);
                    ...
                    //yod2.getSpec().setShieldTexture(Global.getSettings().getSpriteName("industry", "dcp_dysonshell"));
                    ...
                    // First moon.
                    PlanetAPI yod2a = system_five.addPlanet("dcp_planet_yod2a", yod2, "Dzerzhinsky A", "barren", 180, 30, 660, 50f);
                    ...
                    SectorEntityToken stationDerelict4 = DerelictThemeGenerator.addSalvageEntity(system_five, dcp_Entities.STATION_RESEARCH_BREAKER, dcp_Factions.BREAKERS);
                    ...
                    // Second moon.
                    PlanetAPI yod2b = system_five.addPlanet("dcp_planet_yod2b", yod2, "Dzerzhinsky B", "barren-desert", 360*(float)Math.random(), 75, 900, 90f);
		...
		field.setCircularOrbit(five_star, 0, 0, 150);
                ...
                addDerelict(system_five, five_star, "dcp_stormkestrel_proto_test", ShipRecoverySpecial.ShipCondition.GOOD, radius_sparrowhawk, true, "the_deserter");                
                
                //Add a few more derelicts to Kestrel's Rest, including a few prototype Imps (one recoverable).
                addDerelict(system_five, five_star, "dcp_imp_proto_test", ShipRecoverySpecial.ShipCondition.AVERAGE, radius_sparrowhawk - 100f, true, "the_deserter");                
                addDerelict(system_five, five_star, "dcp_imp_proto_test", ShipRecoverySpecial.ShipCondition.WRECKED, radius_sparrowhawk + 125f, true, "the_deserter"); 
                addDerelict(system_five, five_star, "dcp_imp_proto_test", ShipRecoverySpecial.ShipCondition.BATTERED, radius_sparrowhawk + 50f, true, "the_deserter");                
                ...
                addDerelict(system_five, five_star, "dcp_sevastopol_mk1_std", ShipRecoverySpecial.ShipCondition.BATTERED, radius_sparrowhawk + 250f, false, "independent");
                ...
                addDerelict(system_five, five_star, "dcp_puddlejumper_mk1_std", ShipRecoverySpecial.ShipCondition.BATTERED, radius_sparrowhawk + 75F, true, "independent");        
                addDerelict(system_five, five_star, "dcp_puddlejumper_mk1_std", ShipRecoverySpecial.ShipCondition.AVERAGE, radius_sparrowhawk + 25F, false, "independent");        
                
            // Chernov, a lava world with an artificial toposphere. Spooky.
            PlanetAPI yod3 = system_five.addPlanet("dcp_planet_yod3", five_star, "Chernov", "lava_minor", 120, 200, 5200, 400f);
            ...
            PlanetAPI yod3shell = system_five.addPlanet("dcp_planet_yod3shell", five_star, "Artificial Toposphere", "dcp_dysonshell", 120, 240, 5200, 400f);
                ...
                yod3shell.getSpec().setShieldTexture(Global.getSettings().getSpriteName("industry", "dcp_dysonshield"));
                ...
                PlanetAPI yod3a = system_five.addPlanet("dcp_planet_yod3a", yod3, "Chervy", "barren_castiron", 360*(float)Math.random(), 45, 400, 60f);
