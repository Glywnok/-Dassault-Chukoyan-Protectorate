 ...
 */
public class dcp_BonecrackerFX implements EveryFrameWeaponEffectPlugin {
    
    private static final float FIRE_DURATION = 0.2f; // Firing cycle time
    private static final Color FLASH_COLOR = new Color(255,235,200,155);
    private static final float OFFSET = 19f;
    private static final Color PARTICLE_COLOR = new Color(255,235,200,155); // Particle color
    ...
    public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
        ...
        if (weapon.isFiring()) {
            ...
            if (elapsed <= 0f) {
                ...
                engine.spawnExplosion(explosion_offset, ship.getVelocity(), FLASH_COLOR, 25f, 0.15f);
            }
            ...
            int particle_count_this_frame = (int) (14f * (FIRE_DURATION - elapsed));
            for (int x = 0; x < particle_count_this_frame; x++) {
                size = DMEUtils.get_random(3f, 5f);
                speed = DMEUtils.get_random(20f, 80f);
                angle = weapon.getCurrAngle() + DMEUtils.get_random(-24f, 24f);
