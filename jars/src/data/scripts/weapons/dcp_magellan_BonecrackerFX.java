...
public class dcp_magellan_BonecrackerFX implements OnFireEffectPlugin, EveryFrameWeaponEffectPlugin {
   ...
   public void onFire(DamagingProjectileAPI proj, WeaponAPI weapon, CombatEngineAPI engine) {
      ...
      for(int j = 0; j < shotCount1; ++j) {
         ...
         engine.spawnProjectile(proj.getSource(), proj.getWeapon(), "dcp_magellan_bonecracker_sub", loc, proj.getFacing(), weapon_location);
      }
      ...
      for(int j = 0; j < shotCount2; ++j) {
         ...
         engine.spawnProjectile(proj.getSource(), proj.getWeapon(), "dcp_magellan_bonecracker_core", loc, proj.getFacing(), proj_vel);
      }

      engine.removeEntity(proj);
      weapon_location = weapon.getLocation();
      ShipAPI ship = weapon.getShip();
      Vector2f explosion_offset = DCPUtils.translate_polar(weapon_location, 26.0F, weapon.getCurrAngle());
      Vector2f explosion_offset2 = DCPUtils.translate_polar(weapon_location, 22.0F, weapon.getCurrAngle());
   ...
   public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
      if (!engine.isPaused()) {
         if (weapon.isFiring()) {
            ...
            ShipAPI ship = weapon.getShip();
            this.elapsed += amount;
            Vector2f particle_offset = DCPUtils.translate_polar(weapon_location, 19.0F, weapon.getCurrAngle());
            ...
            for(int x = 0; x < particle_count_this_frame; ++x) {
               ...
               float speed = DCPUtils.get_random(20.0F, 80.0F);
               float angle = weapon.getCurrAngle() + DCPUtils.get_random(-18.0F, 18.0F);
