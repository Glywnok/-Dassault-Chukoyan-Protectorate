      ...
      if (!engine.isPaused()) {
         if (weapon.isFiring()) {
            ...
            if (this.elapsed <= 0.0F) {
               particle_offset = DMEUtils.translate_polar(weapon_location, 3.0F, weapon.getCurrAngle());
               engine.spawnExplosion(particle_offset, ship.getVelocity(), FLASH_COLOR, 75.0F, 0.25F);
            }

            this.elapsed += amount;
            particle_offset = DMEUtils.translate_polar(weapon_location, 0.0F, weapon.getCurrAngle());
