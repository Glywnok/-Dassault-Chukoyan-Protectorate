...
public class dcp_DME_PulseripperSmallOnHit extends BaseCombatLayeredRenderingPlugin implements OnHitEffectPlugin {
   public static int NUM_TICKS = 11;
   public static float TOTAL_DAMAGE = 150.0F;
   ...
   protected List<dcp_DME_PulseripperSmallOnHit.ParticleData> particles = new ArrayList();
   ...
   public dcp_DME_PulseripperSmallOnHit() {
      ...
      if (!shieldHit) {
         if (!projectile.isFading()) {
            if (target instanceof ShipAPI) {
               ...
               dcp_DME_PulseripperSmallOnHit effect = new dcp_DME_PulseripperSmallOnHit(projectile, (ShipAPI)target, offset);
               CombatEntityAPI e = engine.addLayeredRenderingPlugin(effect);
               e.getLocation().set(projectile.getLocation());
               engine.addSwirlyNebulaParticle(point, target.getVelocity(), NEBULA_SIZE, 20.0F, 0.25F, 0.2F, 0.8F, NEBULA_COLOR, true);
   ...
   public dcp_DME_PulseripperSmallOnHit(DamagingProjectileAPI proj, ShipAPI target, Vector2f offset) {
      this.layers = EnumSet.of(CombatEngineLayers.BELOW_INDICATORS_LAYER);
      this.proj = proj;
      this.target = target;
      this.offset = offset;
   ...
   public void advance(float amount) {
      if (!Global.getCombatEngine().isPaused()) {
         ...
         List<dcp_DME_PulseripperSmallOnHit.ParticleData> remove = new ArrayList();
         Iterator var4 = this.particles.iterator();

         while(var4.hasNext()) {
            dcp_DME_PulseripperSmallOnHit.ParticleData p = (dcp_DME_PulseripperSmallOnHit.ParticleData)var4.next();
            p.advance(amount);
   ...
   protected void dealDamage() {
      ...
      for(int i = 0; i < num; ++i) {
         dcp_DME_PulseripperSmallOnHit.ParticleData p = new dcp_DME_PulseripperSmallOnHit.ParticleData(30.0F, 3.0F + (float)Math.random() * 2.0F, 2.0F);
   ...
   public void render(CombatEngineLayers layer, ViewportAPI viewport) {
      ...
      while(var7.hasNext()) {
         dcp_DME_PulseripperSmallOnHit.ParticleData p = (dcp_DME_PulseripperSmallOnHit.ParticleData)var7.next();
