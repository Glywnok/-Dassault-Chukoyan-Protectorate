...
public class dcp_magellan_RadFlamerOnHit extends BaseCombatLayeredRenderingPlugin implements OnHitEffectPlugin {
   public static int NUM_TICKS = 11;
   public static float TOTAL_DAMAGE = 100.0F;
   protected List<dcp_magellan_RadFlamerOnHit.ParticleData> particles = new ArrayList();
   ...
   public dcp_magellan_RadFlamerOnHit() {
      ...
      if (!shieldHit) {
         if (!projectile.isFading()) {
            if (target instanceof ShipAPI) {
               ...
               dcp_magellan_RadFlamerOnHit effect = new dcp_magellan_RadFlamerOnHit(projectile, (ShipAPI)target, offset);
   ...
   public dcp_magellan_RadFlamerOnHit(DamagingProjectileAPI proj, ShipAPI target, Vector2f offset) {
      this.layers = EnumSet.of(CombatEngineLayers.BELOW_INDICATORS_LAYER);
      this.proj = proj;
      this.target = target;
      this.offset = offset;
   ...
   public void advance(float amount) {
      if (!Global.getCombatEngine().isPaused()) {
         ...
         List<dcp_magellan_RadFlamerOnHit.ParticleData> remove = new ArrayList();
         Iterator var4 = this.particles.iterator();

         while(var4.hasNext()) {
            dcp_magellan_RadFlamerOnHit.ParticleData p = (dcp_magellan_RadFlamerOnHit.ParticleData)var4.next();
            p.advance(amount);
   ...
   protected void dealDamage() {
      ...
      for(int i = 0; i < num; ++i) {
         dcp_magellan_RadFlamerOnHit.ParticleData p = new dcp_magellan_RadFlamerOnHit.ParticleData(30.0F, 3.0F + (float)Math.random() * 2.0F, 2.0F);
   ...
   public void render(CombatEngineLayers layer, ViewportAPI viewport) {
      ...
      while(var7.hasNext()) {
         dcp_magellan_RadFlamerOnHit.ParticleData p = (dcp_magellan_RadFlamerOnHit.ParticleData)var7.next();
