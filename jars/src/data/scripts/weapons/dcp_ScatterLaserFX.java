...
public class dcp_ScatterLaserFX implements OnFireEffectPlugin, EveryFrameWeaponEffectPlugin {
    ...
    public void onFire(DamagingProjectileAPI proj, WeaponAPI weapon, CombatEngineAPI engine)
    {
        ...
        for (int j = 0; j < shotCount1; j++) {
            ...
            engine.spawnProjectile(proj.getSource(), proj.getWeapon(), "dcp_scatterlaser_sub", loc, proj.getFacing(),
                                   randomVel);
        }
        int shotCount2 = (1); //Core projectile
        for (int j = 0; j < shotCount2; j++) {
            engine.spawnProjectile(proj.getSource(), proj.getWeapon(), "dcp_scatterlaser_core", loc, proj.getFacing(),
                                   null);
        }
        ...
        for (int j = 0; j < shotCount3; j++) {
            ...
            randomVel.y += vel.y;
            //spec + "_clone" means this will call the weapon (not projectile! you need a separate weapon) with the id "($projectilename)_clone".
            engine.spawnProjectile(proj.getSource(), proj.getWeapon(), "dcp_scatterlaser_submicro", loc, proj.getFacing(),
                                   randomVel);
        }
