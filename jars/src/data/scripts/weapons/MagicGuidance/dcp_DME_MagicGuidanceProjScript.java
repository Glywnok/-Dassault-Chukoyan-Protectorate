...
public class dcp_DME_MagicGuidanceProjScript extends BaseEveryFrameCombatPlugin {
   ...
   private static final float TURN_RATE = 300.0F;
   ...
   public dcp_DME_MagicGuidanceProjScript(@NotNull DamagingProjectileAPI proj, CombatEntityAPI target) {
      this.proj = proj;
      this.target = target;
            ...
            if (this.delayCounter < this.actualGuidanceDelay) {
               ...
               if ("DUMBCHASER_SWARM".contains("ONE_TURN") || this.target != null) {
                  ...
                  if ("DUMBCHASER_SWARM".equals("ONE_TURN_DUMB")) {
                     ...
                     facingSwayless += Misc.getClosestTurnDirection(facingSwayless, this.targetAngle) * Math.min(facingSwayless, 300.0F * amount);
                     ...
                     if ("DUMBCHASER_SWARM".equals("ONE_TURN_TARGET")) {
                        facingSwayless1 = this.proj.getFacing() - swayThisFrame;
                        facingSwayless = VectorUtils.getAngle(this.proj.getLocation(), this.targetPoint);

                        for(angleToHit = Math.abs(facingSwayless - facingSwayless1); angleToHit > 180.0F; angleToHit = Math.abs(angleToHit - 360.0F)) {
                        }

                        facingSwayless += Misc.getClosestTurnDirection(facingSwayless, facingSwayless1) * Math.min(angleToHit, 300.0F * amount);
                        this.proj.setFacing(facingSwayless + swayThisFrame);
                        ...
                        this.proj.getVelocity().y = MathUtils.getPoint(new Vector2f(Misc.ZERO), this.proj.getVelocity().length(), facingSwayless1 + swayThisFrame).y;
                     } else {
                        float angleToHit1;
                        if ("DUMBCHASER_SWARM".contains("DUMBCHASER")) {
                           ...
                           angleToHit1 = VectorUtils.getAngle(this.proj.getLocation(), Vector2f.add(this.target.getLocation(), targetPointRotated1, new Vector2f(Misc.ZERO)));

                           for(angleToHit1 = Math.abs(angleToHit1 - facingSwayless); angleToHit1 > 180.0F; angleToHit1 = Math.abs(angleToHit1 - 360.0F)) {
                           }

                           facingSwayless += Misc.getClosestTurnDirection(facingSwayless, angleToHit1) * Math.min(angleToHit1, 300.0F * amount);
                           this.proj.setFacing(facingSwayless + swayThisFrame);
                           this.proj.getVelocity().x = MathUtils.getPoint(new Vector2f(Misc.ZERO), this.proj.getVelocity().length(), facingSwayless + swayThisFrame).x;
                           ...
                           facingSwayless += Misc.getClosestTurnDirection(facingSwayless, angleToHit) * Math.min(angleDiffAbsolute, 300.0F * amount);
                           this.proj.setFacing(facingSwayless + swayThisFrame);
                           this.proj.getVelocity().x = MathUtils.getPoint(new Vector2f(Misc.ZERO), this.proj.getVelocity().length(), facingSwayless + swayThisFrame).x;
                           this.proj.getVelocity().y = MathUtils.getPoint(new Vector2f(Misc.ZERO), this.proj.getVelocity().length(), facingSwayless + swayThisFrame).y;
                        }
   ...
   private void reacquireTarget() {
      ...
      while(var4.hasNext()) {
         ...
         if (potTarget1.getOwner() != this.proj.getOwner() && !(Math.abs(VectorUtils.getAngle(this.proj.getLocation(), potTarget1.getLocation()) - this.proj.getFacing()) > 120.0F) && !potTarget1.isHulk() && !potTarget1.isPhased()) {
