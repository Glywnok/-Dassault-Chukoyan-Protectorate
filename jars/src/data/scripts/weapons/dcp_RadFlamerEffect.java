 ...
 * @author Soren
 */
public class dcp_RadFlamerEffect extends BaseCombatLayeredRenderingPlugin implements OnFireEffectPlugin,
																					OnHitEffectPlugin,
																					EveryFrameWeaponEffectPlugin {

	public dcp_RadFlamerEffect() {
	}
	
	protected List<dcp_RadFlamerEffect> trails;
	public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
		if (trails == null) return;
		
		Iterator<dcp_RadFlamerEffect> iter = trails.iterator();
		while (iter.hasNext()) {
		...
		if (weapon.getShip() != null) {
			...
			for (dcp_RadFlamerEffect curr : trails) {
				if (curr.proj != null) {
		...
		for (int i = 0; i < numIter; i++) { 
			for (dcp_RadFlamerEffect trail : trails) {
...
//								   rampUp, dur, inverted);
	...
	public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
		...
		dcp_RadFlamerEffect trail = new dcp_RadFlamerEffect(projectile, prev);
		...
		if (trails == null) {
			trails = new ArrayList<dcp_RadFlamerEffect>();
		}
		trails.add(0, trail);
	}
...
//			offset.x += vel.x * 1f;
//			offset.y += vel.y * 1f;
			fader = new FaderUtil(0f, 0.2f, 0.4f);
			fader.fadeIn();
		}
	...
	public dcp_RadFlamerEffect(DamagingProjectileAPI proj, DamagingProjectileAPI prev) {
		this.proj = proj;
		this.prev = prev;
	...
	public void render(CombatEngineLayers layer, ViewportAPI viewport) {
		...
		for (ParticleData p : particles) {
			float size = proj.getProjectileSpec().getWidth() * 0.5f;
			size *= p.scale;
