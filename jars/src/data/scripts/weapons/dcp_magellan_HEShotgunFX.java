...
public class dcp_magellan_HEShotgunFX implements OnFireEffectPlugin, EveryFrameWeaponEffectPlugin {
   ...
   public void onFire(DamagingProjectileAPI proj, WeaponAPI weapon, CombatEngineAPI engine) {
      ...
      Vector2f explosion_offset2 = DMEUtils.translate_polar(weapon_location, 30.0F, weapon.getCurrAngle());
   ...
   public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
      if (!engine.isPaused()) {
         if (weapon.isFiring()) {
            ...
            ShipAPI ship = weapon.getShip();
            this.elapsed += amount;
            Vector2f particle_offset = DMEUtils.translate_polar(weapon_location, 27.0F, weapon.getCurrAngle());
            ...
            for(int x = 0; x < particle_count_this_frame; ++x) {
               ...
               float speed = DMEUtils.get_random(15.0F, 120.0F);
               float angle = weapon.getCurrAngle() + DMEUtils.get_random(-15.0F, 15.0F);
