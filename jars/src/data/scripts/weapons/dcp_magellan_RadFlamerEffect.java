...
public class dcp_magellan_RadFlamerEffect extends BaseCombatLayeredRenderingPlugin implements OnFireEffectPlugin, OnHitEffectPlugin, EveryFrameWeaponEffectPlugin {
   protected List<dcp_magellan_RadFlamerEffect> trails;
   protected List<dcp_magellan_RadFlamerEffect.ParticleData> particles = new ArrayList();
   protected DamagingProjectileAPI proj;
   protected DamagingProjectileAPI prev;
   protected float baseFacing = 0.0F;
   protected EnumSet<CombatEngineLayers> layers;

   public dcp_magellan_RadFlamerEffect() {
   ...
   public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
      if (this.trails != null) {
         ...
         while(iter.hasNext()) {
            if (((dcp_magellan_RadFlamerEffect)iter.next()).isExpired()) {
               iter.remove();
            }
         }
         ...
         if (weapon.getShip() != null) {
            ...
            while(var11.hasNext()) {
               dcp_magellan_RadFlamerEffect curr = (dcp_magellan_RadFlamerEffect)var11.next();
               if (curr.proj != null) {
         ...
         for(int i = 0; (float)i < maxRange; ++i) {
            ...
            while(var17.hasNext()) {
               dcp_magellan_RadFlamerEffect trail = (dcp_magellan_RadFlamerEffect)var17.next();
   ...
   public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
      ...
      dcp_magellan_RadFlamerEffect trail = new dcp_magellan_RadFlamerEffect(projectile, prev);
   ...
   public dcp_magellan_RadFlamerEffect(DamagingProjectileAPI proj, DamagingProjectileAPI prev) {
      this.layers = EnumSet.of(CombatEngineLayers.ABOVE_SHIPS_AND_MISSILES_LAYER);
      this.proj = proj;
      this.prev = prev;
      ...
      for(int i = 0; i < num; ++i) {
         this.particles.add(new dcp_magellan_RadFlamerEffect.ParticleData(proj));
      }
      ...
      for(Iterator var7 = this.particles.iterator(); var7.hasNext(); ++index) {
         dcp_magellan_RadFlamerEffect.ParticleData p = (dcp_magellan_RadFlamerEffect.ParticleData)var7.next();
   ...
   public void advance(float amount) {
      if (!Global.getCombatEngine().isPaused()) {
         ...
         while(var2.hasNext()) {
            dcp_magellan_RadFlamerEffect.ParticleData p = (dcp_magellan_RadFlamerEffect.ParticleData)var2.next();
            p.advance(amount);
         }
