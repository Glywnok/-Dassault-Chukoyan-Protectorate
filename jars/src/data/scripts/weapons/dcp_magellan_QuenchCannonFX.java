   ...
   public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
      if (!engine.isPaused()) {
         if (weapon.isFiring()) {
            ...
            ShipAPI ship = weapon.getShip();
            this.elapsed += amount;
            Vector2f particle_offset = DCPUtils.translate_polar(weapon_location, 30.0F, weapon.getCurrAngle());
            ...
            for(int x = 0; x < particle_count_this_frame; ++x) {
               ...
               float speed = DCPUtils.get_random(140.0F, 225.0F);
               float angle = weapon.getCurrAngle() + DCPUtils.get_random(-80.0F, 80.0F);
