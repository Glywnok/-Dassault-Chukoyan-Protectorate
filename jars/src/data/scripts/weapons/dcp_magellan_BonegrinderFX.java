...
public class dcp_magellan_BonegrinderFX implements OnFireEffectPlugin, EveryFrameWeaponEffectPlugin {
   ...
   public void onFire(DamagingProjectileAPI proj, WeaponAPI weapon, CombatEngineAPI engine) {
      ...
      Vector2f explosion_offset = DMEUtils.translate_polar(weapon_location, 33.0F, weapon.getCurrAngle());
      Vector2f explosion_offset2 = DMEUtils.translate_polar(weapon_location, 27.0F, weapon.getCurrAngle());
   ...
   public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
      if (!engine.isPaused()) {
         if (weapon.isFiring()) {
            ...
            ShipAPI ship = weapon.getShip();
            this.elapsed += amount;
            Vector2f particle_offset = DMEUtils.translate_polar(weapon_location, 24.0F, weapon.getCurrAngle());
            int particle_count_this_frame = (int)(10.0F * (0.15F - this.elapsed));

            for(int x = 0; x < particle_count_this_frame; ++x) {
               float size = DMEUtils.get_random(3.0F, 7.0F);
               float speed = DMEUtils.get_random(20.0F, 200.0F);
               float angle = weapon.getCurrAngle() + DMEUtils.get_random(-9.0F, 9.0F);
