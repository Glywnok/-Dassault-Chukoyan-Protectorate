   ...
   public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
      if (!engine.isPaused()) {
         if (weapon.isFiring()) {
            ...
            ShipAPI ship = weapon.getShip();
            this.elapsed += amount;
            Vector2f particle_offset = DMEUtils.translate_polar(weapon_location, 4.0F, weapon.getCurrAngle());
            ...
            for(int x = 0; x < particle_count_this_frame; ++x) {
               ...
               float speed = DMEUtils.get_random(50.0F, 100.0F);
               float angle = weapon.getCurrAngle() + DMEUtils.get_random(-60.0F, 60.0F);
