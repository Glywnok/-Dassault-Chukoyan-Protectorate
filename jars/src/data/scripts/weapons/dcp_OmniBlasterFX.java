...
public class dcp_OmniBlasterFX implements OnFireEffectPlugin, EveryFrameWeaponEffectPlugin {
    ...
    public void onFire(DamagingProjectileAPI proj, WeaponAPI weapon, CombatEngineAPI engine)
    {
        ...
        for (int j = 0; j < shotCount1; j++) {
            ...
            engine.spawnProjectile(proj.getSource(), proj.getWeapon(), "dcp_omniblaster_sub", loc, proj.getFacing(),
                                   randomVel);
        }
        int shotCount2 = (1); //Core projectile
        for (int j = 0; j < shotCount2; j++) {
            engine.spawnProjectile(proj.getSource(), proj.getWeapon(), "dcp_omniblaster_core", loc, proj.getFacing(),
                                   null);
        }
