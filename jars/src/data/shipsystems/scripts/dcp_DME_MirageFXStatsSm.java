...
public abstract class dcp_DME_MirageFXStatsSm implements ShipSystemStatsScript {
   ...
   private static final float PARTICLE_SIZE_MIN = 4.0F;
   ...
   public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
      if (stats.getEntity() instanceof ShipAPI) {
         ...
         if (state == State.IN) {
            ...
            int n_particles = Math.round(21.0F - effectLevel * 21.0F);
            ...
            for(int i = 0; i < n_particles; ++i) {
               ...
               float size = MathUtils.getRandomNumberInRange(4.0F, 8.0F);
               float brightness = (float)Math.random();
               Color color = DCPUtils.lerpRGB(PARTICLE_COLOR_2, PARTICLE_COLOR_1, effectLevel);
