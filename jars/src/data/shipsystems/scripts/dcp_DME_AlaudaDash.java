...
public class dcp_DME_AlaudaDash extends BaseShipSystemScript {
   ...
   public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
      ...
      if (stats.getEntity() instanceof ShipAPI) {
         ...
         id = id + "_" + ship.getId();
         float TimeMult = 1.0F + 3.0F * effectLevel;
         ...
         if (state == State.IN) {
            ship.getMutableStats().getAcceleration().modifyFlat(id, 5000.0F);
            ship.getMutableStats().getDeceleration().modifyFlat(id, 5000.0F);
         } else {
            float speed;
            if (state == State.ACTIVE) {
               ...
               if (speed < 900.0F) {
                  ...
                  ship.getVelocity().scale(speed + driftamount * 3600.0F);
               }
            } else {
               speed = ship.getVelocity().length();
